...
public class InMemoryDataSourceProvider implements DataSourceProvider {
    ...
    public boolean isSuitableFor(ODataRequestContext oDataRequestContext, String entityType) throws ODataDataSourceException {
        return oDataRequestContext.getEntityDataModel().getType(entityType).getJavaType().equals(Datapoint.class);
    }
    ...
    public QueryOperationStrategy getStrategy(ODataRequestContext oDataRequestContext, QueryOperation queryOperation, TargetType targetType) throws ODataException {
        ...
        List<Predicate<Datapoint>> predicateList = builder.buildCriteria(queryOperation, oDataRequestContext);
        ...
        return () -> {
            ...
            Stream<Datapoint> personStream = inMemoryDataSource.getPersonConcurrentMap().values().stream();

            List<Datapoint> filteredPersons = personStream.filter(p -> predicateList.stream()
                    .allMatch(f -> f.test(p))).collect(Collectors.toList());

            long count = 0;
            if (builder.isCount() || builder.includeCount()) {
                count = filteredPersons.size();
                LOG.debug("Counted {} datapoints matching query", count);
            ...
            LOG.debug("Found {} datapoints matching query", filteredPersons.size());

            if (propertyNames != null && !propertyNames.isEmpty()) {
                try {
                    LOG.debug("Selecting {} properties of datapoint", propertyNames);
